...
art::Ptr<simb::MCParticle> NeutrinoSelection::getAssocMCParticle(art::Event const& e,
								 const std::vector<art::Ptr<recob::Hit> >& hits,
								 float& purity, float& completeness) {
 ...
 /*
 size_t NeutrinoSelection::BackTrack(const art::FindMany<simb::MCParticle,anab::BackTrackerHitMatchingData> backtrack_h,
				     const std::vector<unsigned int>& trackid_v,
				     const std::vector<unsigned int>& hit_idx_v, float& completeness, float& purity)
 {
