  ...
  // More initializers here.
{
  ...
  for (size_t i=0;i<_analysisToolsVec.size();i++) _analysisToolsVec[i]->setBranches(_tree);
...
bool NeutrinoSelectionFilter::filter(art::Event & e)
{
  ...
  for (size_t i=0;i<_analysisToolsVec.size();i++) _analysisToolsVec[i]->analyzeEvent(e, fData);//fixme add more arguments, make other functions down the line...
