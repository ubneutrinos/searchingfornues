...
private:
  ...
  /**
     ...
     */
  template <typename T>
  float PFPEnergy(const T &ass_clus_v);
  ...
  std::pair<double,double> CC0piNpSelection::VtxCompatibility(const TVector3& nuvtx, const TVector3& shrvtx, const TVector3& shrdir) {
  ...
  template <typename T> float CC0piNpSelection::PFPEnergy(const T& ass_clus_v) {
  ...
  */
  ...
  float CC0piNpSelection::GetTrackShowerScore(const ProxyPfpElem_t& pfp_pxy) {
