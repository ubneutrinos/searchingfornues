#ifndef ANALYSIS_CRTAPPROACHSEL_CXX
#define ANALYSIS_CRTAPPROACHSEL_CXX
...
namespace analysis
{
    ////////////////////////////////////////////////////////////////////////
    //
    // Class:       CRTApproachAnalysis
    // File:        CRTApproachAnalysis.cc
    //
  ...
  class CRTApproachAnalysis : public AnalysisToolBase {
  ...
  public:
     ...
     *  @param  pset
     */
    CRTApproachAnalysis(const fhicl::ParameterSet& pset);
  ...
  /// pset - Fcl parameters.
  ///
  CRTApproachAnalysis::CRTApproachAnalysis(const fhicl::ParameterSet& pset)
  {
    configure(pset);
  }
  ...
  ///
  void CRTApproachAnalysis::configure(fhicl::ParameterSet const & pset)
  {
    fTrackAssnModuleLabel = pset.get< std::string >  ("TrackAssnModuleLabel","trackmatch");

  }

  void CRTApproachAnalysis::analyzeEvent(art::Event const& e, bool fData)
  {
    _evt = e.event();
    _sub = e.subRun();
    _run = e.run();
  ...
  //----------------------------------------------------------------------------
  ///
  void CRTApproachAnalysis::analyzeSlice (art::Event const& e, std::vector<ProxyPfpElem_t>& slice_pfp_v, bool fData, bool selected)
  {
    std::cout<<"--------------------------------------------------- \n";
    Reset();
  ...
  void CRTApproachAnalysis::resetTTree(TTree* _tree)
  {
  ...
  double CRTApproachAnalysis::Point2PointDistance(const TVector3& nuvtx, TVector3& taggedTrackSpt){
    return  (nuvtx-taggedTrackSpt).Mag();
  }
  


  void CRTApproachAnalysis::Reset() {
    _nAssnCosmics = -1;
  ...
  DEFINE_ART_CLASS_TOOL(CRTApproachAnalysis)
} // namespace analyze
