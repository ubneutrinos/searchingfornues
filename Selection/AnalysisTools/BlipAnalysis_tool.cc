...
private:
  ...
  std::vector<bool>	   _blip_neutron_flag; //was blip created by particle originating from final state neutron
...
//    _blip_trkid.push_back(blipVec[i].TrkID);
	...
	for (int i_mcp = 0; i_mcp < (int) mcparticle.size(); i_mcp++)
	{
	  if (mcparticle[i_mcp]->PdgCode()==2112 && mcparticle[i_mcp]->Mother()==0)
          {
            if (BlipUtils::IsAncestorOf(mcparticle[blip_to_mcpar]->TrackId(),mcparticle[i_mcp]->TrackId(), false)==1)
	    {
	      b_flag=true;
	    }
	  }
    ...
    _blip_trkid.push_back(blipVec[i].truth.LeadG4ID);
    ...
    _blip_neutron_flag.push_back(b_flag);
...
void BlipAnalysis::fillDefault()
{
  ...
  _blip_neutron_flag.push_back(false);
...
void BlipAnalysis::setBranches(TTree *_tree)
{
  ...
  _tree->Branch("blip_neutron_flag","std::vector< bool >", &_blip_neutron_flag);
}
